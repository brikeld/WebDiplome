...
- Image pipeline: each generation pass picks one random unused image from the Electron app's `assets/recent_images` + `assets/screenshots`, copies it into `public/uploads/` under a SHA-256 name, and feeds the base64 to the model's vision input. Personas cycle through which one gets the image (`IMAGE_PERSONA_CYCLE`). If the model lacks vision, the prompt is downgraded to a filename-only text reference.
...
- `public/uploads/{sha256}.{ext}` — content-addressed user-uploaded and generator-imported images. Served at `/uploads/...` by `server.js`. `.gitignore`d.
...
4. renderer/app.js uploads attached images to ─────────► POST /api/upload  (server.js :3001)
...
User in the web UI clicks "Generate new content" ─────► POST /api/posts/generate (server-generate.js :3010)
                                                        ...
                                                        plus picks a random image from
                                                        Electron's assets/{recent_images,screenshots}.
                                                        New posts are prepended to posts/{id}.json.
```

Concretely:

1. **Initial sync (Electron → WebDiplome HTTP).** `renderer/app.js` constructs a `syncPayload` from the Flask `/analyze` result + locally-generated posts and `POST`s it to `http://localhost:3001/api/profile`. Attached images on posts go through `POST /api/upload` first, and the returned `/uploads/...` URLs are spliced back into the payload's `personaPosts[*].attachedImage.url`. See `renderer/app.js` `syncProfileToWebDiplome()` and `uploadAttachedImagesForPosts()`.
...
4. **Hardcoded absolute paths.** `server-generate.js` references `/Users/brikeld/Documents/Repo/Diplome_/testCreationAcc/...` directly. If you move either repo, update `ELECTRON_DATA_DIR` and `EXTRA_ASSET_DIRS` at the top of `server-generate.js`. There is no environment-variable fallback for these paths today.
...
- **Images are content-addressed.** Never trust the original filename; always hash and dedupe. `scripts/dedupe-uploads.js` reconciles `public/uploads/` if something gets out of sync.
